...
{
	struct despotify_session* ds = (struct despotify_session*)arg;

	DSFYINFO("Networking thread started\n");
	...
	DSFYINFO("Networking thread exiting\n");
...
{
	struct despotify_session* ds = (struct despotify_session*)calloc(1,sizeof(struct despotify_session));
	if (!ds)
		return NULL;
		...
		unsigned short len)
{
	if (ch->state == CHANNEL_DATA) {
		struct despotify_session* ds = (struct despotify_session*)ch->private_storage;
		struct track* t = ds->track;

		if (t->key)
			DSFYfree(t->key);

		t->key = (unsigned char*)malloc(len);
		...
		struct track* copy = (struct track *)malloc(sizeof(struct track));
	...
	case CHANNEL_END:
		...
		if (ch->total_data_len < SUBSTREAM_SIZE) {
			...
			if (ds->track && ds->play_as_list) {
				unsigned char fid[20], tid[16];
				hex_ascii_to_bytes((char*)ds->track->file_id, fid, sizeof fid);
				hex_ascii_to_bytes((char*)ds->track->track_id, tid, sizeof tid);
...
struct search_result* despotify_search(struct despotify_session* ds,
	...
	ds->response = buf_new();
	ds->playlist = (struct playlist*)calloc(1, sizeof(struct playlist));
	...
	if (!ds->playlist->tracks)
		ds->playlist->tracks = (struct track*)calloc(1, sizeof(struct track));

	struct buf* b = despotify_inflate(ds->response->ptr, ds->response->len);
	if (b) {
		search = (struct search_result*)calloc(1, sizeof(struct search_result));
...
struct search_result* despotify_search_more(struct despotify_session *ds,
	...
	if (b) {
		...
		t = t->next = (struct track*)calloc(1, sizeof(struct track));
...
static bool despotify_load_tracks(struct despotify_session *ds, bool cache_do_store)
{
	...
	unsigned char* tracklist = (unsigned char*)malloc(MAX_BROWSE_REQ * 16);
	SHA1_CTX sha_ctx;
	...
	int count;
	int totcount;
	for (totcount=0; totcount < pl->num_tracks; totcount += count) {
		ds->response = buf_new();

		struct track* firsttrack = t;
		for (count = 0; t && count < MAX_BROWSE_REQ; t = t->next, count++)
			hex_ascii_to_bytes((char*)t->track_id, tracklist + count * 16, 16);
		...
		for (t = pl->tracks; t; t = t->next) {
			if (!t->has_meta_data) {
				...
				for (tt = pl->tracks; tt; tt = tt->next) {
					if (tt->has_meta_data &&
							!strncmp((char*)tt->track_id, (char*)t->track_id, sizeof tt->track_id)) {
						struct track* next = t->next;
						*t = *tt;
						t->next = next;

						/* deep copy of artist list */
						struct artist* a = (struct artist*)calloc(1, sizeof(struct artist));
						t->artist = a;
						struct artist* ta;
						for (ta = tt->artist; ta; ta = ta->next) {
							*a = *ta;
							if (ta->next)
								a = a->next = (struct artist*)calloc(1, sizeof(struct artist));
						}

						/* deep copy of georestrictions. */
						if(tt->allowed) {
							t->allowed = (char*)calloc(strlen(tt->allowed) + 1,
									sizeof(char));
						...
						if(tt->forbidden) {
							t->forbidden = (char*)calloc(strlen(tt->forbidden) + 1,
									sizeof(char));
...
struct playlist* despotify_get_playlist(struct despotify_session *ds,
	...
	if (ds->use_cache && playlist_id && cache_contains(playlist_id)) {
		...
		if ((data = cache_load(playlist_id, &len)) != NULL) {
			...
			DSFYstrncpy((char*)ds->playlist->playlist_id, (char*)playlist_id, sizeof ds->playlist->playlist_id);
	...
	if (playlist_id)
		hex_ascii_to_bytes((char*)playlist_id, pid, 17);
	else {
...
struct playlist* despotify_get_stored_playlists(struct despotify_session *ds)
{
	...
	struct playlist* p;
	for (p = metalist; p && p->playlist_id[0]; p = p->next) {
		DSFYDEBUG("Requesting playlist with ID '%s'\n", p->playlist_id);
		struct playlist* new_playlist = despotify_get_playlist(ds, p->playlist_id, old_use_cache);
		if (prev)
			prev->next = new_playlist;
		else
			root = new_playlist;
		prev = new_playlist;
	}
...
struct artist_browse* despotify_get_artist(struct despotify_session* ds,
		...
		{
	ds->artist_browse = (struct artist_browse*)calloc(1, sizeof(struct artist_browse));
	...
	unsigned char id[16];
	hex_ascii_to_bytes((char*)artist_id, id, sizeof id);
...
void* despotify_get_image(struct despotify_session* ds, unsigned char* image_id, unsigned int* len)
{
	/* check cache */
	...
	unsigned char id[20];
	hex_ascii_to_bytes((char*)image_id, id, sizeof id);
...
struct album_browse* despotify_get_album(struct despotify_session* ds,
		...
		{
	ds->album_browse = (struct album_browse*)calloc(1, sizeof(struct album_browse));
	...
	unsigned char id[16];
	hex_ascii_to_bytes((char*)album_id, id, sizeof id);
...
struct track* despotify_get_tracks(struct despotify_session* ds, char* track_ids[], int num_tracks)
{
	...
	/* construct an array of 16-byte track ids */
	unsigned char* tracklist = (unsigned char*)malloc(num_tracks * 16);
...
struct playlist* despotify_link_get_playlist(struct despotify_session* ds, struct link* link)
{
	...
	struct playlist *playlist = despotify_get_playlist(ds, (unsigned char*)buf, old_use_cache);
...
char* despotify_album_to_uri(struct album_browse* album, char* dest)
{
	...
	snprintf(dest, 64, "spotify:album:%s", uri);
...
char* despotify_artist_to_uri(struct artist_browse* artist, char* dest)
{
	...
	snprintf(dest, 64, "spotify:artist:%s", uri);
...
char* despotify_playlist_to_uri(struct playlist* playlist, char* dest)
{
	...
	stpncpy(id, (char*)playlist->playlist_id, 32);
	id[32] = '\0';
	despotify_id2uri(id, uri);

	snprintf(dest, 64, "spotify:user:%s:playlist:%s", playlist->author, uri);
...
{
	snprintf(dest, 64, "spotify:search:%s", search->query);
...
char* despotify_track_to_uri(struct track* track, char* dest)
{
	...
	snprintf(dest, 64, "spotify:track:%s", uri);
