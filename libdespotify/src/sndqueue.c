...
bool snd_init(struct despotify_session *ds)
{
	...
	ds->fifo = (struct snd_fifo*)calloc(1, sizeof(struct snd_fifo));
	if (!ds->fifo)
		return false;
...
void snd_ioctl (struct despotify_session* ds, int cmd, void *data, int length)
{
	...
	struct snd_buffer* buff = (struct snd_buffer*)malloc(sizeof(struct snd_buffer));
	if (!buff) {
...
{
	struct despotify_session* ds = (struct despotify_session*)session;
	...
	while (loop) {
		...
		DSFYDEBUG_SNDQUEUE("Processing one buffer at ds->fifo->start."
				" %d items of size %d requested. Totbytes: %d\n",
...
int snd_get_pcm(struct despotify_session* ds, struct pcm_data* pcm)
{
	...
	if (!ds->vf) {
		if(ds->fifo->totbytes < 4096) {
			// Decoder throws a 'no Vorbis' error with too little data.
			pcm->len = 0;
			shortsleep();
			return 0;
		}
		...
		int ret = ov_open_callbacks(ds, (OggVorbis_File*)ds->vf, NULL, 0, callbacks);
		if (ret) {
			DSFYfree(ds->vf);
			DSFYINFO("ov_open_callbacks(): error %d (%s)\n",
					ret,
...
//		if(ds->fifo->totbytes < (ds->fifo->maxbytes - SUBSTREAM_SIZE)) {
	...
	vorbis_info* vi = ov_info((OggVorbis_File*)ds->vf, -1);
	...
	while (1) {
		/* decode to pcm */
		ssize_t r = ov_read((OggVorbis_File*)ds->vf, pcm->buf, sizeof(pcm->buf),
		...
		if (ds->client_callback) {
			double point = ov_time_tell((OggVorbis_File*)ds->vf);
